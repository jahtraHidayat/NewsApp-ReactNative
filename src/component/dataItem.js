import React, {Component} from 'react';
import {
  ListItem,
  Left,
  Thumbnail,
  Text,
  Body,
  Right,
  Button,
} from 'native-base';
import {View, StyleSheet} from 'react-native';
import TimeAgo from './time'; // import from component/time.js

class DataItem extends Component {
  constructor(props) {
    super(props);
    this.data = props.data; // restore to props.data
  }

  handlePress = () => {
    const {url, title} = this.data; // return to the item in tab1.js
    this.props.onPress({url, title});
  };

  render() {
    return (
      <ListItem thumbnail>
        <Left>
          <Thumbnail
            square
            source={{
              uri:
                this.data.urlToImage != null
                  ? this.data.urlToImage
                  : 'data:image/png;base64,',
            }}
          />
        </Left>
        <Body>
          <Text numberOfLines={2}>{this.data.title}</Text>
          <Text note numberOfLines={2}>
            {this.data.description}
          </Text>
          <View style={styles.labelDescription}>
            <Text note>{this.data.source.name}</Text>
            <TimeAgo time={this.data.publishedAt} />
          </View>
        </Body>
        <Right>
          <Button transparent onPress={this.handlePress}>
            <Text>View</Text>
          </Button>
        </Right>
      </ListItem>
    );
  }
}

export default DataItem;

const styles = StyleSheet.create({
  labelDescription: {
    flex: 1,
    flexDirection: 'row',
    marginTop: 8,
    marginLeft: 0,
  },
});
